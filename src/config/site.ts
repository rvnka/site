import type { SiteConfig, SocialLink } from '@/types';

const profile = {
  name: 'Ringga',
  username: '@rvnka',
  avatar: 'https://avatars.githubusercontent.com/u/99320495',
  bio: 'I am a beginner developer, editor and student who enjoys learning new things and breaking stuff. I build things out of curiosity, interest, and the fun of seeing ideas turn into something real.',
  email: 'someone@example.com',
  startYear: 2020,
  location: 'Southeast Asia',
  available: false,
} as const;

const meta = {
  title: profile.name,
  description: `${profile.name} - portfolio.`,
  url: 'https://site.rvnka.my.id',
  icon: '/api/brand?type=icon',
  ogImage: '/api/brand?type=og',
} as const;

const roles = ['Developer', 'Editor', 'Student', 'Anime'] as const;

const nav = [
  { label: 'Home',    href: '/' },
  { label: 'About',   href: '/about' },
  { label: 'Projects', href: '/project' },
  { label: 'Blog',    href: '/blog' },
  { label: 'Contact', href: '/contact' },
] as const;

const social = [
  { label: 'GitHub',  href: 'https://github.com/rvnka',                          icon: 'bi-github',   username: 'rvnka' },
  { label: 'YouTube', href: 'https://www.youtube.com/@rvnka_yt',                 icon: 'bi-youtube',  username: '@rvnka_yt' },
  { label: 'Discord', href: 'https://discord.com/users/1007123238985412608',      icon: 'bi-discord',  username: 'ringga' },
  { label: 'Email',   href: profile.email ? `mailto:${profile.email}` : '',      icon: 'bi-envelope', username: profile.email },
] as const;

const skills = [
  { name: 'HTML',       icon: 'bi-filetype-html', color: '#e34f26' },
  { name: 'CSS',        icon: 'bi-css',           color: '#264de4' },
  { name: 'JavaScript', icon: 'bi-javascript',    color: '#f7df1e' },
  { name: 'Node.js',    icon: 'bi-server',        color: '#339933' },
  { name: 'TypeScript', icon: 'bi-typescript',    color: '#3178c6' },
  { name: 'Bash',       icon: 'bi-filetype-sh',   color: '#4eaa25' },
  { name: 'Python',     icon: 'bi-filetype-py',   color: '#3776ab' },
  { name: 'Git',        icon: 'bi-git',           color: '#f05032' },
  { name: 'Linux',      icon: 'bi-tux',           color: '#fcc624' },
  { name: 'Android',    icon: 'bi-android',       color: '#3ddc84' },
] as const;

const timeline = [
  {
    year: '2020',
    title: 'First Line of Code',
    description: 'Started learning HTML & CSS out of curiosity. Built my first static webpage and got hooked. Learned JavaScript fundamentals, and built small interactive projects.',
  },
  {
    year: '2021',
    title: 'The Penguin (Tux) and The Snake (Python)',
    description: 'Surfing the internet and discovered Linux, get hooked to Shell & Bash scripting and Python.',
  },
  {
    year: '2022-2023',
    title: 'A Frameworks? And Ecosystem? What is that?',
    description: 'Dived into Node.js and discovered the power of JavaScript. Started learning and creating.',
  },
  {
    year: '2024-Present',
    title: 'What now?, idk.',
    description: 'Creating some projects~',
  },
] as const;

const education = [
  {
    year: '2008 - 2018',
    institution: 'Early Childhood Education and Elementary School',
    description: 'Developed foundational social and academic skills during the formative years.',
  },
  {
    year: '2020 - 2023',
    institution: 'Junior High School',
    description: 'Balanced core academic requirements while discovering a passion for digital technology.',
  },
  {
    year: '2024 - Present',
    institution: 'Vocational High School',
    description: 'Pursuing creative storytelling and cinematography while maintaining a self-taught engineering path.',
  },
] as const;

export const siteConfig = {
  meta,
  profile,
  roles,
  nav,
  social,
  skills,
  timeline,
  education,
} as const satisfies SiteConfig;

export const activeSocial = siteConfig.social.filter(
  (s) => Boolean(s.href) && s.href !== '#',
) as readonly SocialLink[];

export default siteConfig;

export function hasValidEmail(email?: string): boolean {
  return Boolean(email && email.includes('@'));
}

export function hasLocation(location?: string): boolean {
  return Boolean(location?.trim());
}
